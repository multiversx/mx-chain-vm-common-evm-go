...
import (
	...
	"github.com/multiversx/mx-chain-vm-common-evm-go/mock"
)
...
func BuildTestEncodingContext() *EncodingContext {
	return BuildEncodingContext(&mock.EVMAccountsStub{
		RequestAddressCalled: func(request *evmcommon.AddressRequest) (*evmcommon.AddressResponse, error) {
			err := evmcommon.ValidateAddressRequest(request)
			if err != nil {
				return nil, err
			}
			...
			if request.SourceIdentifier == core.MVXAddressIdentifier {
				if request.RequestedIdentifier == core.ETHAddressIdentifier {
					...
					return &evmcommon.AddressResponse{MultiversXAddress: request.SourceAddress, RequestedAddress: ethereumAddress}, nil
				}
			}
			...
			if request.SourceIdentifier == core.ETHAddressIdentifier {
				if request.RequestedIdentifier == core.MVXAddressIdentifier {
					...
					return &evmcommon.AddressResponse{MultiversXAddress: multiversXAddress, RequestedAddress: multiversXAddress}, nil
				}
			}
